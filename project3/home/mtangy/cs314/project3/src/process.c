...
static int total_Tickets = 0;                                             // Global total ticket var

static  int interactiveNum;                                       //distinguishes between interactive processes        
static  int interactiveStart = 0;   
static  int interactiveEnd = 0;                                           //timing variables
static  int interactiveTotal = 0;
...
ProcessModuleInit ()
{
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    pcbs[i].tickets = 0;  // Initialize total tickets for each PCB to 0
...
ProcessSchedule ()
{
  ...
  int           cur_Ticket;  //sum of traversed tickets
  PCB*         PCB_iter; 
  PCB*           pcb2;
  int            numOfProcesses;
  int            processCount;
...
//  printf("process tickets:%d\n", pcb->tickets);
  ...
  numOfProcesses = QueueLength(&runQueue);
  pcb = (PCB *)((QueueFirst (&runQueue))->object);
  ...
  while(processCount <= numOfProcesses){
    ...
    processCount++;
    //pcb = (PCB *)(PCB_iter->object);
    cur_Ticket += pcb->tickets;

    if(winner < cur_Ticket){break;}
    ...
    //PCB_iter = (QueueNext(PCB_iter).l)->next;

  }


  if(pcb->interactiveFlag = 1){

    interactiveTotal = TimerGet() - interactiveTotal;
    printf(TIMESTRING1, interactiveNum);
    printf(TIMESTRING2, interactiveTotal);
    printf(TIMESTRING2, interactiveNum, pcb->tickets);    
  ...
  dbprintf ('p',"About to switch to PCB 0x%x,flags=0x%x @ 0x%x\n",
            pcb, pcb->flags,pcb->sysStackPtr[PROCESS_STACK_IAR]);
...
ProcessSuspend (PCB *suspend)
{
  ...
  total_Tickets = total_Tickets - suspend->tickets;      //Remove suspend process's tickets from total tickets 
...
ProcessFork (VoidFunc func, uint32 param, int p_nice, int p_info,char *name, int isUser)
{
  ...
  if(p_nice ==15){
    ...
    pcb->interactiveFlag = 1;
  }

  interactiveNum = p_info;
